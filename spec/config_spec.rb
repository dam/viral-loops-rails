...
describe VLoopsRails do
  it 'possess a default global configuration' do
    ...
    expect(VLoopsRails.config).to eq(api_token: nil, campaign_id: nil, debug: false)
  end

  it 'gets/sets global configuration' do
    VLoopsRails.configure(api_token: '1234', campaign_id: 1)
  ...
  it 'filters extra parameters that are not valid' do
    ...
    expect(VLoopsRails.config).to eq(api_token: '5678', campaign_id: 2, debug: false)
  end
end
